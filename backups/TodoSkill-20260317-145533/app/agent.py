...
13. 像“添加待办-这是一个测试任务创建时间为2026年3月15日17点23分32秒”这种句子，intent 必须是 `create`，title 是任务标题，`created_at` 是 `2026年3月15日17点23分32秒`。
14. 像“#16任务完成时间为2026年3月16日20点30分00秒”这种句子，intent 必须是 `done`，`task_id` 是 16，`completed_at` 是 `2026年3月16日20点30分00秒`。
""".strip()

    try:
        response = llama_client.chat(
            system_prompt="你是 Todo 指令路由器。你的职责是理解用户真实意图，并输出可执行 JSON。",
            user_prompt=prompt,
            temperature=0.1,
            max_tokens=600,
        )
